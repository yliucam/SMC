...
foreach(jj = 1:nrow(PARAMS)) %dopar% {
  try({
    ...
    source("Sys_resamp.R")
    source("dc_melding_11sub_jags_stage_one.R")
    source("dc_melding_11sub_smc_stage_one.R")
    
    PATH_PREFIX <-  "/nesi/project/uoa03349/postdoc/dc_melding/11_submodel/results/sub11/"
    PARAM <- PARAMS[jj,]
    sub_n <- PARAM$sub
    repN <- PARAM$rep_N
    ...
    x7[1] <- exp(rnorm(1, phi67, x7_0))
    y7 <- integer(10)
    y7[1] <- rnorm(1, phi78, x7[1])
    for (t in 2:10) {
      x7[t] <- exp(rnorm(1, phi67, x7[t-1]))
    ...
    data <- list(phi12 = phi12, phi23 = phi23, phi34 = phi34, phi45 = phi45,
                 ...
                 psi6 = psi6, psi8 = psi8, psi9 = psi9, psi10 = psi10,
                 y1 = y1, y2 = y2, y3 = y3, y4 = y4, y5 = y5, y6 = y6, 
                 y7 = y7, y8 = y8, y9 = y9, y10 = y10, y11 = y11)
    
    out <- dc_melding_smc_sub11(data = y11,
                                mu_prior = 0,
                                sigma_prior = 2,
                                alpha_prior = 2,
                                beta_prior = 2,
