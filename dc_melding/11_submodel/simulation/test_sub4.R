...
foreach(jj = 1:nrow(PARAMS)) %dopar% {
  try({
    ...
    PATH_PREFIX <-  "/nesi/project/uoa03349/postdoc/dc_melding/11_submodel/results/sub4/"
    PARAM <- PARAMS[jj,]
    sub_n <- PARAM$sub
    repN <- PARAM$rep_N
    ...
    FILENAME_SUB2 <- paste0("RES_n50_submodel2_repN", repN_s[jj], ".RData")
    FILENAME_SUB3 <- paste0("RES_n50_submodel3_repN", repN_s[jj], ".RData")
    FILENAME_SUB5 <- paste0("RES_n50_submodel5_repN", repN_s[jj], ".RData")
    FILEADDRESS_SUB2 <- paste0("/nesi/project/uoa03349/postdoc/dc_melding/11_submodel/results/sub2/", FILENAME_SUB2)
    ...
    out <- dc_melding_smc_sub4(data = y4,
                               out_sub3 = out3,
                               out_sub5 = out5,
                               N = 10000,
                               n_sub = 2,
