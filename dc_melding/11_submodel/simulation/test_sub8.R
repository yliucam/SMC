...
foreach(jj = 1:nrow(PARAMS)) %dopar% {
  try({
    ...
    source("dc_melding_11sub_jags_stage_three.R")
    source("dc_melding_11sub_smc_stage_three.R")
    
    PATH_PREFIX <-  "/nesi/project/uoa03349/postdoc/dc_melding/11_submodel/results/sub8/"
    PARAM <- PARAMS[jj,]
    sub_n <- PARAM$sub
    repN <- PARAM$rep_N
    ...
    FILENAME_SUB10 <- paste0("RES_n50_submodel10_repN", repN_s[jj], ".RData")
    FILENAME_SUB9 <- paste0("RES_n50_submodel9_repN", repN_s[jj], ".RData")
    FILENAME_SUB7 <- paste0("RES_n50_submodel7_repN", repN_s[jj], ".RData")
    FILEADDRESS_SUB10 <- paste0("/nesi/project/uoa03349/postdoc/dc_melding/11_submodel/results/sub10/", FILENAME_SUB10)
    FILEADDRESS_SUB9 <- paste0("/nesi/project/uoa03349/postdoc/dc_melding/11_submodel/results/sub9/", FILENAME_SUB9)
    ...
    out <- dc_melding_smc_sub8(data = y8,
                               out_sub7 = out7,
                               out_sub9 = out9,
                               N = 10000,
                               n_sub = 2,
