...
foreach(jj = 1:nrow(PARAMS)) %dopar% {
  try({
    ...
    source("dc_melding_11sub_jags_stage_four.R")
    source("dc_melding_11sub_smc_stage_four.R")
    
    PATH_PREFIX <-  "/nesi/project/uoa03349/postdoc/dc_melding/11_submodel/results/sub6/"
    PARAM <- PARAMS[jj,]
    sub_n <- PARAM$sub
    repN <- PARAM$rep_N
    ...
    out <- dc_melding_smc_sub6(data = y6,
                               out_sub5 = out5,
                               out_sub7 = out7,
                               N = 10000,
                               n_sub = 2,
