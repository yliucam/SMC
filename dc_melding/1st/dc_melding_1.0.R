# Used for psi3 being Normal distributed
...
dc_melding_1st <- function(data,
  ...
  p_check_log_mN <- apply(x_leaf_mN, 1, function(x) dnorm(x[1], (4*x[1]+x[2])/9, 1, log = T) + dnorm(x[2], (4*x[1]+x[2])/9, 2, log = T))
  p_check_log_mN <- alpha*p_c_prod_log_mN + (1-alpha)*p_check_log_mN
  ...
  for (i in 1:(nt-1)) {
    if (i == 1) {
      ### Update weights
      #p_root_0_log <- p_check_log + apply(cbind(x_root_0, x_leaf_merge), 1, function(x) dnorm(x[1], (4*x[2]+x[3])/9, 2/3, log = T))
      #p_root_0_log <- p_check_log + sapply(x_root_0, function(x) dnorm(x, 0, 1, log = T))
      #p_root_t_log <- apply(cbind(x_leaf_merge, x_root_0), 1, function(x) dnorm(x[1], x[3], 1, log = T) + dnorm(x[2], x[3], 2, log = T))
      #p_root_t_log <- p_root_t_log + sapply(x_root_0, function(x) dnorm(x, 0, 1, log = T))
      #p_root_log_previous <- p_root_0_log
      #p_root_log_current <- (1-alpha_update)*p_root_0_log + alpha_update*p_root_t_log 
      #w_root_log[,i] <- w_root_log_0 + p_root_log_current - p_root_log_previous
      p_root_log <- apply(cbind(x_leaf_merge, x_root_0), 1, function(x) dnorm(x[1], x[3], 1, log = T) + dnorm(x[2], x[3], 2, log = T))
      w_root_log[,i] <- w_root_log_0 + alpha * (p_root_log - p_check_log)
      ...
      ### No need to update using a Markov kernel here! - the toy example is too simple
    ...
    ### Update weights
    #p_root_0_log <- p_check_log + apply(cbind(x_root, x_leaf_merge), 1, function(x) dnorm(x[1], (4*x[2]+x[3])/9, 2/3, log = T))
    #p_root_0_log <- p_check_log + sapply(x_root, function(x) dnorm(x, 0, 1, log = T))
    #p_root_t_log <- apply(cbind(x_leaf_merge, x_root), 1, function(x) dnorm(x[1], x[3], 1, log = T) + dnorm(x[2], x[3], 2, log = T))
    #p_root_t_log <- p_root_t_log + sapply(x_root, function(x) dnorm(x, 0, 1, log = T))
    #p_root_log_previous <- (1-alpha_update)*p_root_0_log + alpha_update*p_root_t_log
    #### Update alpha
    alpha_update <- alpha_update + alpha
    #p_root_log_current <- (1-alpha_update)*p_root_0_log + alpha_update*p_root_t_log
    #w_root_log[,i+1] <- w_root_log[,i] + p_root_log_current - p_root_log_previous
    p_root_log <- apply(cbind(x_leaf_merge, x_root), 1, function(x) dnorm(x[1], x[3], 1, log = T) + dnorm(x[2], x[3], 2, log = T))
    w_root_log[,i+1] <- w_root_log[,i] + alpha * (p_root_log - p_check_log)
