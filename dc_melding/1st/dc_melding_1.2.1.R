...
dc_melding_1.2.1 <- function(data,
                             N,
                             n_leaf,
                             m,
                             alpha,
  ...
  ncores <- parallel::detectCores() - 1  # Use all available cores minus one
  cl <- makeCluster(ncores)              # Create a cluster
  registerDoParallel(cl)                 # Register the cluster
  
  out_leaf <- foreach(leaf_i = 1:n_leaf, .export = "dc_melding_leaf_mcmc") %dopar% {
    dc_melding_leaf_mcmc(data = data[,leaf_i],
                         N = N,
                         Ntotal = Ntotal_init)
  }
  ...
  for (leaf_i in 1:n_leaf) {
    x_leaf[,leaf_i] <- out_leaf[[leaf_i]]$mu
    w_leaf_log[,leaf_i] <- out_leaf[[leaf_i]]$w_log
    W_leaf[,leaf_i] <- out_leaf[[leaf_i]]$W
  }
  ...
  u_c_log_mN <- normal_nodes_log_sum_new(x_leaf_mN, mu = matrix(rep(0, N*m*n_leaf), ncol=n_leaf), sigma = matrix(rep(1, N*m*n_leaf), ncol=n_leaf))
  p_c_marginal_log_mN <- normal_nodes_log_sum_new(x_leaf_mN, mu = matrix(rep(x_leaf_mN_means, N*m), nrow=N*m, byrow=T), sigma = matrix(rep(phi_sigma, N*m), nrow=N*m, byrow=T))
  ...
  for (i in 1:(nt-1)) {
    if (i == 1) {
      ### Update weights
      p_root_log <- normal_nodes_log_sum_new(x_leaf_merge, mu = matrix(rep(x_root_0, n_leaf), ncol=n_leaf), sigma = matrix(rep(phi_sigma, N), ncol=n_leaf, byrow=T))
      p_root_log <- p_root_log + exponential_psd_log_sum(data_0, lambda = x_root_0)
      ...
      if (max(w_root_log[,i]) < min_lim) w_root_log[,i] <- w_root_log[,i]/1e4
      ...
      ### MCMC kernel
      out_root <- dc_melding_root_mcmc(data=data_0, data_phi=x_leaf_merge, Ntotal=Ntotal, mu_init=x_root_resamp, phi_sigma=phi_sigma, alpha_j=alpha_update)
      x_root <- out_root$mu[,Ntotal]
      x_root_array[,i] <- x_root
    }
    
    ### Update weights
    p_root_log <- normal_nodes_log_sum_new(x_leaf_merge, mu = matrix(rep(x_root, n_leaf), ncol=n_leaf), sigma = matrix(rep(phi_sigma, N), ncol=n_leaf, byrow=T))
    p_root_log <- p_root_log + exponential_psd_log_sum(data_0, lambda = x_root)
