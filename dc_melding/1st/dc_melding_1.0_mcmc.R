...
# MCMC on root
dc_melding_root_mcmc <- function(data, Ntotal, mu_init, alpha_j) {
  N <- dim(data)[1]
  mu <- array(rep(NA, N*Ntotal), dim = c(N, Ntotal))
  for (i in 1:(Ntotal-1)) {
    if (i == 1) {
      mu_star <- sapply(mu_init, function(x) rnorm(1, x, 10))
      
      #prior_log <- sapply(mu_init, function(x) dnorm(x, 0, 1, log = T))
      #prior_star_log <- sapply(mu_star, function(x) dnorm(x, 0, 1, log = T))
      
      u_t_log <- sapply(mu_init, function(x) dnorm(x, 0, 1, log = T))
      u_t_star_log <- sapply(mu_star, function(x) dnorm(x, 0, 1, log = T))
      
      likelihood_log <- likelihood_star_log <- rep(NA, N)
      for (j in 1:N) {
        likelihood_log[j] <- dnorm(data[j, 1], mu_init[j], 1, log = T) + dnorm(data[j, 2], mu_init[j], 2, log = T)
        likelihood_star_log[j] <- dnorm(data[j, 1], mu_star[j], 1, log = T) + dnorm(data[j, 2], mu_star[j], 2, log = T)
      }
      
      post_log <- (1-alpha_j) * u_t_log + alpha_j * likelihood_log
      post_star_log <- (1-alpha_j) * u_t_star_log + alpha_j * likelihood_star_log
      #post_log <- 0 * prior_log + likelihood_log
      #post_star_log <- 0 * prior_star_log + likelihood_star_log
    ...
    #prior_log <- sapply(mu[,i], function(x) dnorm(x, 0, 1, log = T))
    #prior_star_log <- sapply(mu_star, function(x) dnorm(x, 0, 1, log = T))
    u_t_log <- sapply(mu[,i], function(x) dnorm(x, 0, 1, log = T))
    u_t_star_log <- sapply(mu_star, function(x) dnorm(x, 0, 1, log = T))
    
    likelihood_log <- likelihood_star_log <- rep(NA, N)
    for (j in 1:N) {
      likelihood_log[j] <- dnorm(data[j, 1], mu[j,i], 1, log = T) + dnorm(data[j, 2], mu[j,i], 2, log = T)
      likelihood_star_log[j] <- dnorm(data[j, 1], mu_star[j], 1, log = T) + dnorm(data[j, 2], mu_star[j], 2, log = T)
    }
  ...
  w_log <- apply(cbind(data, mu[,Ntotal]), 1, function(x) dnorm(x[1], x[3], 1, log = T) + dnorm(x[2], x[3], 2, log = T))
  W <- exp(w_log - matrixStats::logSumExp(w_log))
