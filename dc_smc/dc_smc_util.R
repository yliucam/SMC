...
sub_mcmc <- function(data, x_0, prior_alpha, prior_beta, like_beta, var_sub, Ntotal) {
  ...
  for (i in 1:(Ntotal-1)) {
    if (i == 1) {
      ...
      x_star <- sapply(x_0, function(x) rlnorm(1, log(x), 2.38*sqrt(var_sub)))
      ...
      prop_x_to_x_star <- apply(x_and_x_star, 1, function(x) dlnorm(x[2], log(x[1]), 2.38*sqrt(var_sub), log = T))
      prop_x_star_to_x <- apply(x_and_x_star, 1, function(x) dlnorm(x[1], log(x[2]), 2.38*sqrt(var_sub), log = T))
    ...
    # Proposal - log-normal random walk
    x_star <- sapply(x, function(x) rlnorm(1, log(x), 2.38*sqrt(var_sub)))
